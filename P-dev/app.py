...
import os
import yaml
from user_summary_functions import get_user_summary, get_basic_metrics, get_top_categories, get_liked_by_users, get_users_engagement_chart
...
from flask import request  # Ensure this is imported

@app.route("/<course_name>/get_specific_users_stat", methods=["POST"])
def specific_users_stat(course_name):
    course_name = course_name.replace("-", "_").replace(":","_")
    print(f"Course name = {course_name}")
    # Retrieve the user list from the form data
    user_list = request.form.get("user_list")
    if not user_list:
        flash("No usernames provided. Please enter at least one username.")
        return redirect(url_for('course_page', course_name=course_name))

    # Process the user list and generate the chart
    user_list = user_list.split(",")  # Convert comma-separated string to a list
    chart = get_users_engagement_chart(course_name, user_list)

    viz_folder = os.path.join("static", "visualizations")  # Folder for visualizations
    os.makedirs(viz_folder, exist_ok=True)
    chart_path = url_for('static', filename='visualizations/temp.html')
    chart.save(os.path.join(viz_folder, 'temp.html'))
    print("Chart saved to ", chart_path)

    latest_visualizations_html_path = url_for('static', filename=f'visualizations/course_specific_charts/t1_2025/{course_name}.html')
    
    return render_template('course_specific_viz.html', 
                           course_name=course_name.title().replace("_", " "),
                           latest_visualizations_html_path = latest_visualizations_html_path, 
                           specific_users_chart=chart_path)
